...
--   9
--
--   Maximum is a built in function that takes a list and
--   returns the max value of the set. Therefore it would 
--   work recursively with maxInt. Found by searching 
--   maximum haskell on google lol.
maxInt :: Tree -> Int
maxInt (Leaf i) = i
...
--   1
--
--   minimum is a built in function that takes a list and
--   returns the min value of the set. Therefore it would 
--   work recursively with minInt.
minInt :: Tree -> Int
minInt (Leaf i) = i
...
--   [6,2,1,4,3,5,8,7,9]
--   
--   The ++ operator is the list concatenation operator which takes two lists as operands and 
--   "combine" them into a single list. So if you have the list [x] and the list [y] then you 
--	 can concatenate them like this: [x]++[y] to get [x, y].
preorder :: Tree -> [Int]
preorder (Leaf i) = [i]
...
inBST :: Int -> Tree -> Bool
inBST x (Leaf i) = i == x
inBST x (Node i l r) = x == i || inBST x l || inBST x r